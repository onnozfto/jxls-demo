package com.self.jxlsdemo.command;

import com.self.jxlsdemo.JxlsBuilder;
import com.self.jxlsdemo.JxlsImage;
import com.self.jxlsdemo.JxlsUtil;
import java.io.File;
...
public class ImageCommand extends AbstractCommand {
    ...
    public Command addArea(Area area) {
        if( super.getAreaList().size() >= 1){
            throw new IllegalArgumentException("You can add only a single area to 'image' com.self.jxlsdemo.command");
        }
        this.area = area;
    ...
    public Size applyAt(CellRef cellRef, Context context) {
        if( area == null ){
            throw new IllegalArgumentException("No area is defined for image com.self.jxlsdemo.command");
        }
